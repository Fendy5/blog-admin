<template>
  <div>
    <section class="f-card q-pa-lg row justify-between">
      ...
      <div>
        <q-btn color="primary" @click="getTagList" class="mr-8" label="搜索" />
        <q-btn flat text-color="black" @click="resetForm" label="重置" />
      </div>
    </section>
...
export default defineComponent({
  setup () {
    ...
    function getTagList () {
      void getTagListApi({ ...page, name: search.name }).then(value => {
        tableData.rows = value.data.data
      })
    }
    ...
    return { search, columns, tableData, page, handleDelete, handleEdit, addTag, getTagList, resetForm }
  }
})
